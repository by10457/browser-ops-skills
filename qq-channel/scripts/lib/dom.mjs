...
export function snapshotDOM(s){
 ...
 const detail=m?.[2]?{id:m[2],url:u.href,channelId:m[1],boardId:u.searchParams.get('subc'),boardName:t(document,'.feed-info__detail__from-name'),author:{name:t(document,'.game-guild-detail-poster-userInfo .user-info__name-text'),avatar:q(document,'.game-guild-detail-poster-userInfo img')?.getAttribute('src')||null},text:rich(q(document,'.short-textcontent-container-full'))||rich(q(document,'.feed-content > .long-textcontent-container')),publishedLabel:t(document,'.feed-info__detail__time'),images:all(document,'.feed-content > .short-textcontent-container-full img').filter(i=>i.alt!=='emoji').map(i=>i.getAttribute('src')),like:{state:likeState,icon:icon||null,label:likeLabel},comments,commentsReady:!!q(document,'.comment-bar__comment-count,.has-no-comment'),commentsEnd:!!q(document,'.has-no-more-comment,.has-no-comment')}:null;
 if(detail){detail.textComplete=!!q(document,'.short-textcontent-container-full');detail.warnings=detail.textComplete?[]:['unsupported-detail-text-layout'];}
...
export async function clickUnique(page,selector){const all=await page.$$(selector);if(all.length!==1)fail('CONTROL_NOT_UNIQUE','控件缺失或不唯一：'+selector);if(!await all[0].isVisible())fail('CONTROL_HIDDEN','控件不可见');await all[0].click();}
